...
*/
function dtril2triu( order, M, N, k, A, LDA, B, LDB ) {
	var isrm;
	var sa1;
	var sa2;
	var sb1;
	...
	if ( l === null ) {
		throw new TypeError( format( '36XFx', order ) );
	}
	isrm = isRowMajor( l );
	if ( isrm ) {
	...
	if ( LDA < max( 1, sa ) ) {
		throw new RangeError( format( '36XJL', sa, LDA ) );
	}
	if ( LDB < max( 1, sb ) ) {
		throw new RangeError( format( '36XGM', sb, LDB ) );
	}
	if ( isrm ) {
		sa1 = LDA;
		sa2 = 1;
